import type { PerformanceStats, VideoAssSubtitleOptions, WrassFontSource, WrassRendererEvent } from './types'

export type WrassWorkerRequest =
...
export type WrassWorkerResponse =
  ...
  | { id: number; type: 'ok' }
  | { id: number; type: 'event'; event: WrassRendererEvent }
...
type WrassWorkerRequestWithoutId =
...
export class AssRendererWorkerClient {
  ...
  private readonly pending = new Map<number, { resolve: (value: WrassWorkerResponse) => void; reject: (error: Error) => void }>()
  private readonly onEvent?: (event: WrassRendererEvent) => void
  ...
  constructor(options: AssRendererWorkerClientOptions) {
    ...
    this.worker.addEventListener('message', (event: MessageEvent<WrassWorkerResponse>) => this.handleMessage(event.data))
  ...
  async renderAt(time: number, force = false): Promise<WrassWorkerResponse> {
  ...
  destroy(): void {
    ...
    for (const { reject } of this.pending.values()) reject(new Error('wrass worker client destroyed'))
    this.pending.clear()
  }

  private send(message: WrassWorkerRequestWithoutId, transfer: Transferable[] = []): Promise<WrassWorkerResponse> {
    if (this.destroyed) return Promise.reject(new Error('wrass worker client is destroyed'))
    const id = this.nextId++
    const request = { id, ...message } as WrassWorkerRequest
  ...
  private handleMessage(response: WrassWorkerResponse): void {
